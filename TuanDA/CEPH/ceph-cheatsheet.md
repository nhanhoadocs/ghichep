...
- Xóa pool :
    - Thêm đoạn `mon allow pool delete = true` vào file `/ceph-deploy/ceph.conf`. 
    - sau đó restart lại service MON
    ```sh
    ceph osd pool delete {pool-name} {pool-name} --yes-i-really-really-mean-it
    ```
    - Bỏ cấu hình đã thêm và restart lại service MON
...
### Nguồn: https://github.com/uncelvel/tutorial-ceph/blob/master/docs/operating/ceph-cheat-sheet.md
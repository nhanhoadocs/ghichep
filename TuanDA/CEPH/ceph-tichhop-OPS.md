...
```sh
ceph auth get-or-create client.cinder | ssh 172.16.0.243 sudo tee /etc/ceph/ceph.client.cinder.keyring
ceph auth get-or-create client.cinder | ssh 172.16.0.244 sudo tee /etc/ceph/ceph.client.cinder.keyring
ceph auth get-or-create client.cinder | ssh 172.16.0.245 sudo tee /etc/ceph/ceph.client.cinder.keyring

ceph auth get-or-create client.cinder-backup | ssh 172.16.0.243 sudo tee /etc/ceph/ceph.client.cinder-backup.keyring
ceph auth get-or-create client.cinder-backup | ssh 172.16.0.244 sudo tee /etc/ceph/ceph.client.cinder-backup.keyring
ceph auth get-or-create client.cinder-backup | ssh 172.16.0.245 sudo tee /etc/ceph/ceph.client.cinder-backup.keyring
```

- Chuyển key cinder sang các node Compute
```sh
ceph auth get-or-create client.cinder | ssh 172.16.0.246 sudo tee /etc/ceph/ceph.client.cinder.keyring
ceph auth get-or-create client.cinder | ssh 172.16.0.247 sudo tee /etc/ceph/ceph.client.cinder.keyring

#Copy doan ma hoa vao de gen secret o buoc sau
ceph auth get-key client.cinder | ssh 172.16.0.246 tee /root/client.cinder
ceph auth get-key client.cinder | ssh 172.16.0.247 tee /root/client.cinder
```
...
> Output 
```sh 
8734bda9-23fe-498d-89d2-f54abe723926
```
...
<uuid>8734bda9-23fe-498d-89d2-f54abe723926</uuid>
<usage type='ceph'>
...
```sh
[root@compute1 ceph]# sudo virsh secret-define --file ceph-secret.xml
Secret 8734bda9-23fe-498d-89d2-f54abe723926 created
```

- Gán giá trị của `client.cinder` cho `uuid`
```sh 
virsh secret-set-value --secret 8734bda9-23fe-498d-89d2-f54abe723926 --base64 $(cat /root/client.cinder)
```
> Output 
```sh 
Secret value set   
...
rbd_user = cinder
rbd_secret_uuid = 8734bda9-23fe-498d-89d2-f54abe723926

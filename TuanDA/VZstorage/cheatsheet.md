...
If you need to change target’s IP address, stop the target as described in Stopping Virtuozzo Storage iSCSI Targets, then run the command 
vstorage-iscsi set -t <target_name> -a <new_IP_address>.

# <a name="changelunsize"> Thay đổi LUN size </a>

If you need to increase the size of a LUN, stop the target as described in Stopping Virtuozzo Storage iSCSI Targets, then run the command 
vstorage-iscsi lun-grow -t <target_name> -l <lun_ID> -s <new_size>.


vstorage -c vsto rm-cs --cancel 1030


/usr/libexec/vstorage/prepare_vstorage_drive /dev/sdh --noboot
/usr/libexec/vstorage/prepare_vstorage_drive /dev/sdf --ssd


vstorage -c vsto advise-configuration -w \
--cs /vstorage/682bd60a \
--cs /vstorage/c2c75342 \
--ssd /vstorage/99ca671f -m /mnt/vstorage


# cau hinh khong cache
## format o cung
...
## phai xoa rong thu muc cs - khong tao se loi 
...
**Doi voi o da co cache can drop cache:**
...
### Mount storage - khong Cache (read)
...
You have the following setup: http://prntscr.com/o4oe82
